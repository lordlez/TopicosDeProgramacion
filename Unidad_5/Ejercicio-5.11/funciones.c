#include "header.h"

int cargar_archivo_bin1(char *rutaBin1)
{
    t_empleados empleado[] =
    {
        {15, "A", "A", 1200},
        {40, "A", "A", 650},
        {10, "B", "C", 850},
        {20, "B", "C", 340},
        {8, "W", "M", 475}
    };

    FILE *pf;
    pf = fopen(rutaBin1, "wb");
    if(pf==NULL)
        return -1;
    fwrite(empleado, sizeof(empleado), 1, pf);
    fclose(pf);
    return 1;
}

int cargar_archivo_bin2(char *rutaBin2)
{
    t_estudiantes estudiante[] =
    {
        {5, "A", "A", 10},
        {15, "A", "A", 3},
        {70, "B", "A", 9},
        {10, "B", "C", 8},
        {20, "B", "C", 8},
        {60, "J", "P", 5},
        {6, "W", "M", 2}
    };

    FILE *pf;
    pf = fopen(rutaBin2, "wb");
    if(pf == NULL)
        return -2;
    fwrite(estudiante, sizeof(estudiante), 1, pf);
    fclose(pf);
    return 1;
}




int cargar_archivo_txt_variable1(char *rutaDestino, char *rutaOrigen)
{
    t_empleados emp;
    FILE *pfBin, *pfTxt;

    pfBin = fopen(rutaOrigen, "rb");
    if(pfBin == NULL)
        return -1;

    pfTxt = fopen(rutaDestino, "wt");
    if(pfTxt == NULL)
    {
        fclose(pfBin);
        return -2;
    }

    fread(&emp, sizeof(t_empleados), 1, pfBin);
    while(!feof(pfBin))
    {
        fprintf(pfTxt, "APELLIDO: %3s, NOMBRE: %3s, DNI: %3d, SUELDO: %3.2f\n", emp.apellido, emp.nombre, emp.dni, emp.sueldo);
        fread(&emp, sizeof(t_empleados), 1, pfBin);
    }
    fclose(pfBin);
    fclose(pfTxt);

    return 1;
}

int cargar_archivo_txt_variable2(char *rutaDestino, char *rutaOrigen)
{
    t_estudiantes est;
    FILE *pfBin, *pfTxt;

    pfBin = fopen(rutaOrigen, "rb");
    if(pfBin == NULL)
        return -1;

    pfTxt = fopen(rutaDestino, "wt");
    if(pfTxt == NULL)
    {
        fclose(pfBin);
        return -2;
    }

    fread(&est, sizeof(t_estudiantes), 1, pfBin);
    while(!feof(pfBin))
    {
        fprintf(pfTxt, "APELLIDO: %3s, NOMBRE: %3s, DNI: %3d, PROMEDIO: %3.2f\n",est.apellido, est.nombre, est.dni, est.promedio);
        fread(&est, sizeof(t_estudiantes), 1, pfBin);
    }

    fclose(pfBin);
    fclose(pfTxt);

    return 1;
}




